...
class ImagemRedondaWidget extends StatelessWidget {
  final isOnline;
  ...
  const ImagemRedondaWidget({Key key, this.isOnline = false, this.showRanking = false, this.ranking, @required this.imagePath, this.name = ""})
      : super(key: key);
...
class RoundedImageBorder extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isOnline) {
      borderPaint.shader = gradiente.createShader(Rect.fromCircle(center: center, radius: size.width / 2));
    } else {
      borderPaint.color = terciariaTexto;
    }
